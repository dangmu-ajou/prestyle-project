"""
내장 카메라 PPE 추적 스크립트 (신체 5존 버전)
사람(yolov8n) + 헬멧 + 조끼 + 장갑(좌/우 분리) + 안전화 (Yolov8s 모델)

[개선 사항]
  - torso 존 y_top 0.28로 하향 → 머리/목 영역 vest 오탐 방지
  - head 존 imgsz=320 → 멀리 있을 때 작은 crop도 고해상도 추론
  - 존별 min_px 체크 → 너무 작은 crop은 skip (오탐/노이즈 방지)
  - 장갑 좌/우 존 분리 → 한 손씩 개별 탐지
  - confidence 재조정: helmet 낮춤(거리 탐지), vest 높임(오탐 억제)
"""
...
# ── 신체 존 정의 ─────────────────────────────────────────────────
# y_top/y_bot   : person bbox 상단 기준 비율 (0.0 = 머리 꼭대기)
# x_start/x_end : 수평 중심(cx) 기준 ±(비율 × H), 음수=왼쪽
# imgsz         : 이 존 추론 시 사용할 해상도 (head는 320으로 고해상도)
# min_px        : crop의 최소 픽셀 크기 — 이보다 작으면 skip
BODY_ZONES = {
    "head": {
        "y_top":   0.00,
        "y_bot":   0.28,   # 헬멧 + 이마까지 포함
        "x_start": -0.13,
        "x_end":    0.13,
        "ppe":     ["helmet"],
        "imgsz":   320,    # 작은 crop도 고해상도 추론 → 거리 탐지 개선
        "min_px":  22,
    },
    "torso": {
        "y_top":   0.30,   # 목/턱 아래부터 시작 → 머리 vest 오탐 방지
        "y_bot":   0.68,
        "x_start": -0.16,
        "x_end":    0.16,
        "ppe":     ["vest"],
...
PPE_CONF_MAP = {
    "helmet":       0.45,   # 거리 탐지 위해 다소 낮춤
    "vest":         0.65,   # 머리/얼굴 오탐 방지 위해 높임
    "gloves":       GLOVE_CONF,
    "safety_boots": 0.55,
}
...
def get_zone_crop(frame, person_box, zone_key):
    """
    비대칭 x_start/x_end 기반 crop.
    min_px 미만이면 None 반환 (노이즈 방지).
    """
...
def run():
    ...
    print("=" * 50)
    print("PPE 추적 시작 (내장 카메라)  q → 종료")
    print(f"감지 PPE: {list(ppe_models.keys())}")
    print("=" * 50)

    WIN_NAME = "PPE Zone Tracking (Built-in)"
